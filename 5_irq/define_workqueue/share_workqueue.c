...
struct workqueue_struct * test_userdef_workqueue;/* 用户自定义一个工作队列 */
/**
 * @brief 共享工作队列回调函数
 * 
 * @param work 
...
void testwork_func(struct work_struct *work){
    msleep(1000);
    // container_of(work, struct args, struct work_struct);
...
irqreturn_t gpio0_c5_interrupt_handler(int irq, void * args)
{
    ...
    queue_work(test_userdef_workqueue, &test_share_work_queue);/* 自定义工作队列的调度 */
...
static int __init irq_request_init(void)
{
    ...
    test_userdef_workqueue = create_workqueue("user_define_workqueue");/* 初始化一个自定义工作队列 */
    return 0;
}

static void __exit irq_request_exit(void)
{
    cancel_work_sync(&test_share_work_queue);/* 先取消后刷新让其快速之恶行，然后再进行销毁 */
    flush_workqueue(test_userdef_workqueue);
    destroy_workqueue(test_userdef_workqueue);
    free_irq(irq, NULL);
