...
struct property * myget_property;
...
int platform_driver_probefunc (struct platform_device *dev)
{
    ...
    myget_property = of_find_property(my_node, "compatible", &size);
    printk(KERN_INFO" get info property is :%s",myget_property->name);
    num = of_property_count_elems_of_size(my_node, "reg", 4);
    printk(KERN_INFO" get the property \"reg\" element number is %d", num);
    ret = of_property_read_u32_index(my_node, "reg", 0, &out_value);
    ret = of_property_read_u64_index(my_node, "reg", 1, &out_value_u64);

    of_property_read_variable_u32_array(my_node, "reg", out_value_array, 1, 2);
    printk(KERN_INFO" get the property \"reg\" element1 number is %x", out_value_array[0]);
    printk(KERN_INFO" get the property \"reg\" element2 number is %x", out_value_array[1]);
    return 0;
}
